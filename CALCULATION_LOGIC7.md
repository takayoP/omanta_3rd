...
- **`equity`（純資産）**: 最新のFY実績データから取得
  ...
  - 定義: 純資産（JGAAP）または親会社所有者帰属持分（IFRS）。概念は近いが厳密に同一ではない場合がある
  - 用途: PBRの計算に使用
...
- **`JOIN latest l ON fs.code = l.code AND fs.current_period_end = l.current_period_end`**: (code, current_period_end)のペアでJOINすることで、別銘柄の期末日が混ざることを理論上防止
...
Step2では`(code, current_period_end)`のペアでJOINすることで、別銘柄の期末日が混ざることを理論上防止します。
`IN (...)`を使うと、別銘柄の期末日が混ざる（cross join的な取り違え）リスクが残りますが、
JOINを使うことでこのリスクを排除します。
...
概念は近いが厳密に同一ではない場合があります。月次スクリーニング用途では大抵は許容されますが、
比較可能性を上げるなら、J-Quantsのフィールド定義（どの持分か）を確認して、ドキュメントに明記しておくと将来迷いません。

### 11. _split_multiplier_betweenの権利落ち日が休場の場合

**境界条件の取り方**

`_split_multiplier_between`関数では、`date > start_date AND date <= end_date`の範囲で`adjustment_factor != 1`を拾います。

権利落ちが祝日等で実際の売買日が翌営業日になる場合、J-Quants側がどの日付にfactorを入れるか次第で
「境界の取り方」で取り漏れが起き得ます。

**保守的な回避策**

- `price_date`は`snap_price_date`で営業日に寄せている（既にOK）
- `date > start_date`は維持（FY期末当日の調整は通常不要、権利落ちは期末後に来る想定）
- 取り漏れ検知のため、調整日を記録（必要に応じてログ出力可能）

通常は問題ありませんが、念のため調整日を記録しています。
