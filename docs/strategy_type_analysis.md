# 戦略タイプの分析：長期保有型 vs 月次リバランス型

## 問題提起

`optimize_longterm.py`（長期保有型の最適化）が`optimize_timeseries.py`（月次リバランス型の最適化）の`_run_single_backtest_portfolio_only`を使用している点について、混同がないか確認が必要です。

## 結論：**意図的な共用（混同ではない）**

### 理由

1. **`_run_single_backtest_portfolio_only`は「ポートフォリオ選定のみ」を行う関数**
   - パフォーマンス計算は行わない
   - `build_features` + `select_portfolio`を実行するだけ

2. **ポートフォリオ選定ロジックは共通**
   - 長期保有型も月次リバランス型も、同じ`build_features` + `select_portfolio`を使用
   - ポートフォリオ選定のロジック自体に違いはない

3. **違いは「パフォーマンス計算方法」のみ**
   - 長期保有型：固定ホライズン評価（`horizon_months`）
   - 月次リバランス型：月次リターン系列（ti→ti+1）
...
# - 固定ホライズン評価（horizon_months）
# - calculate_portfolio_performanceを使用
# - 長期保有型の評価指標を計算
```
...
# - calculate_timeseries_returnsを使用
# - 月次リバランス型の評価指標を計算
```
...
1. **`optimize_longterm.py`が正しく長期保有型の評価を行っているか**
   - `calculate_longterm_performance`内で`horizon_months`を使用しているか
   - 固定ホライズン評価が正しく実装されているか

2. **`optimize_timeseries.py`が正しく月次リバランス型の評価を行っているか**
   - `calculate_timeseries_returns`を使用しているか
   - 月次リターン系列が正しく計算されているか
