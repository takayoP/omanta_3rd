...
これは、以前のスコア比例ウェイト戦略の**選定ロジック**が優れている可能性を示していますが、**「優れている前提」ではなく「優れているか検証するため」**に今回の変更を実施します。

**注意**: -2.54%になったのは「旧best paramsが比例ウェイト向けに最適化されていた」ことの証拠にはなりますが、それだけで「選定ロジックが優れている」とは言えません（重みの付け方の効果で良かった可能性も大きい）。
...
- **比較ルート（`compare_lambda_penalties.py`）**: `select_portfolio`（`longterm_run.py`）を使用 ← **不一致**
- **本番運用（`batch_longterm_run.py`）**: `select_portfolio`（`longterm_run.py`）を使用 ← **不一致**

**問題点**: 最適化している戦略と、実際に運用する戦略が違う状態。この状態で再最適化しても、最終的に本番に載せた瞬間に「思ったのと違う」が再発する可能性があります。
...
**必須確認**: optimize/compare/本番で同一条件の `portfolio_hash` が一致することを確認してください。

確認スクリプト: `verify_portfolio_consistency.py` を使用してください。
