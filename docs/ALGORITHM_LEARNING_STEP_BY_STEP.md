...
- **探索しているもの**：そのスコアを**どう混ぜて・何本選ぶか**（PolicyParams：entry_share, top_n, sector_cap など）。

まずは「データ → 特徴量 → スコア → 選定 → 評価」の順で進む、とだけ押さえておけば十分です。
...
**optimize_strategy** は、**PolicyParams の 6 個**だけを Optuna で探索します。
...
4. **最適化（パラメータを探したいとき）**  
   `optimize_strategy --start 開始日 --end 終了日 --n-trials 数`。  
   → 上で説明した「PolicyParams の探索」が走る。
...
3. **短い期間で最適化**  
   `optimize_strategy --start 2023-01-01 --end 2023-12-31 --n-trials 5` で、PolicyParams が変わるごとに objective がどう変わるかを見る。

不明な用語や「この Step をもう少し詳しく」があれば、`algorithm_calculation_logic_mindmap.mm` や `docs/V1_SLIM_IMPLEMENTATION_SUMMARY.md` とあわせて参照すると整理しやすいです。
