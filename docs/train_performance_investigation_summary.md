...
**実装状況**: `calculate_longterm_performance`に`max_eval_end_used`の出力を追加済み（`src/omanta_3rd/jobs/optimize_longterm.py`の481-500行目）。
...
**実装状況**: `re_evaluate_old_params.py`スクリプトを作成済み。Windowsでの並列処理エラーを回避するため、`n_jobs=1`で実行するように修正済み。
...
1. **`re_evaluate_old_params.py`を実行**: Windowsでの並列処理エラーを回避するため、`n_jobs=1`で実行するように修正済み
2. **実際のログを確認**: 次回の最適化実行時に、`calculate_longterm_performance`のログから除外件数と最大eval_endを確認
...
- Optunaの動作は正常 ✅
- `train_dates`は同じ ✅
- 旧best paramsの再評価スクリプトは作成済み（並列処理エラーを修正済み） ✅
- 実際のログでの確認は次回の最適化実行時に実施予定
