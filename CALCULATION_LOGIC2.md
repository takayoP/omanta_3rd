...
### ステップ1: 累積調整係数（CAF）の計算

```python
def _calculate_cumulative_adjustment_factor(conn, code, target_date):
    """
    評価日（target_date）より後のAdjustmentFactorの累積積を計算
    
    例: 
    - 2025-10-09に1:3分割（adjustment_factor = 0.333333）が発生
    - 評価日が2025-12-19の場合、CAF = 1.0（評価日より後の分割はないため）
    - 評価日が2025-10-08の場合、CAF = 0.333333（2025-10-09の分割を考慮）
    """
    # target_dateより後のAdjustmentFactorを取得
    # 累積積を計算: CAF = ∏(adjustment_factor)
    return caf
```

**重要なポイント:**
- CAFは「その日より後のAdjustmentFactorの累積積」を計算
- 評価日より後に分割が発生している場合、CAF < 1.0
- 評価日より後に分割が発生していない場合、CAF = 1.0

**実際の計算例（コード7419、評価日2025-12-19）:**
- 2025-10-09に1:3分割が発生（adjustment_factor = 0.333333）
- 評価日2025-12-19より後の分割はないため、CAF = 1.0

### ステップ2: 評価日時点の株数の計算（FY期末→評価日の分割・併合を考慮）
...
**重要なポイント:**
- `adj_close`は調整後終値（分割を考慮して調整された価格）
- `shares_adjunit`は調整後株価の基準に合わせた株数
- 時価総額 = 調整後終値 × 調整後株価の基準に合わせた株数

**例:**
- `adj_close` = 1,179円
- `shares_at_price_date` = 95,784,798株（FY期末株数 × 分割倍率）
- `market_cap` = 1,179 × 95,784,798 = 112,930,277,842円
...
**例:**
- `market_cap` = 112,930,275,663円
- `profit` = 32,292,000,000円
- `per` = 112,930,275,663 / 32,292,000,000 = 3.50
...
- `pbr` = 112,930,275,663 / 208,307,000,000 = 0.54

### ステップ6: Forward PERの計算
...
- `forward_per` = 112,930,275,663 / 40,000,000,000 = 2.82
...
```sql
SELECT disclosed_date, current_period_end, profit, equity, eps, bvps,
...
- その後、`current_period_end`が最新のレコードを選択
...
FROM fins_statements
WHERE disclosed_date <= 評価日
ORDER BY disclosed_date DESC, period_priority
```

**重要なポイント:**
- `disclosed_date <= 評価日`の条件で、評価日以前に開示されたデータのみを使用
- `type_of_current_period = 'FY'`を優先（`period_priority`で制御）
