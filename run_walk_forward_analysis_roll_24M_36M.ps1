# Walk-Forward Analysis実行スクリプト（24M/36M同時実行）
# 逆張り許可設計で24M/36Mのrollテストを同時に実行します

Write-Host "=" * 80
Write-Host "Walk-Forward Analysis 実行（24M/36M同時実行）"
Write-Host "=" * 80
Write-Host ""
...
$END_DATE = "2025-12-31"
$TRAIN_MIN_YEARS = 2.0
$N_TRIALS = 30  # 初期は30、時間があれば100に変更可能
$STUDY_TYPE = "C"
...
$log24M = "walk_forward_24M_$timestamp.log"
$log36M = "walk_forward_36M_$timestamp.log"
