    ...
    if not forecast.empty:
        ...
        if not price_data.empty:
            ...
            if not latest_shares.empty:
                ...
                print(f"Forward PER（調整後予想EPS使用）: {forward_per_calc:.6f}")
                
                # 予想EPSを調整（CAFで掛ける）
                adjusted_forecast_eps2 = forecast_eps * caf if caf != 1.0 else forecast_eps
                print(f"\n予想EPS（調整後、CAFで掛ける）: {adjusted_forecast_eps2:.2f}円")
                forward_per_calc2 = market_cap / adjusted_forecast_eps2
                print(f"Forward PER（調整後予想EPS使用）: {forward_per_calc2:.6f}")
                
                # 従来の計算方法
                forward_per_old = price / forecast_eps
                print(f"\n従来のForward PER（価格 / 予想EPS）: {forward_per_old:.6f}")
