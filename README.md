...
- **入替頻度**：高頻度（月次で全入れ替え）
- **特徴**：検証のために、実運用の制約（現金余力・証券会社制約）を反映した **実運用方式（月末始値で売り、月初始値で買う）** の売買定義を採用
- **データ保存先**：
...
#### 実運用方式（証券会社制約による売買タイミング）
- **意思決定**：リバランス日 `t` の時点で新ポートフォリオ（上位N銘柄）を確定
- **売却**：リバランス日 `t`（月末）の始値（open）で全決済
- **購入**：次リバランス日 `t_next`（月初）の始値（open）で新規購入
- **期間リターン**：`open(t) → open(t_next)`  
- **TOPIXも同じタイミング**（月末始値 / 月初始値）で統一
...
- **Python**: 3.9 以上
- **インストール**: リポジトリルートで `pip install -e .` を実行
- **設定**: J-Quants API 利用時は `.env` に `JQUANTS_REFRESH_TOKEN` 等を設定（詳細は `SYSTEM_SPECIFICATION.md` 参照）
- **DB**: 初回は `init-db` でデータベースを初期化。DB パスは環境により異なります（デフォルト例: `%LOCALAPPDATA%\omanta_3rd\db\jquants.sqlite`）
...
本リポジトリには、互換性のための **旧方式（累積リターン系）** と、推奨の **時系列方式（実運用方式：月末始値で売り、月初始値で買う）** が共存します。
